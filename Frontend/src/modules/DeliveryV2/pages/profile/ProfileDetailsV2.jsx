...
/**
 * ProfileDetailsV2 - 1:1 EXACT Restoration of the Legacy UI.
 * This version includes the Flutter Camera Bridge and exact HTML/CSS structure.
 */
export const ProfileDetailsV2 = () => {
  ...
  useEffect(() => {
    const refreshWalletBalance = async () => {
      try {
        ...
        const wallet = (data?.success && data?.data?.wallet) || data?.wallet || data?.data || data
        const balance = Number(wallet?.totalBalance || wallet?.balance || wallet?.pocketBalance || 0) || 0
    ...
    return () => window.removeEventListener("deliveryWalletStateUpdated", refreshWalletBalance)
  }, [])
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-poppins">
        ...
        >
          <div className="space-y-4 pb-4">
            ...
            <div>
              ...
              />
              {docEditFiles.aadharPhoto && <p className="text-xs text-green-600 mt-1">{docEditFiles.aadharPhoto.name}</p>}
            </div>
            <div>
              ...
              />
              {docEditFiles.panPhoto && <p className="text-xs text-green-600 mt-1">{docEditFiles.panPhoto.name}</p>}
            </div>
            <div>
              ...
              />
              {docEditFiles.drivingLicensePhoto && <p className="text-xs text-green-600 mt-1">{docEditFiles.drivingLicensePhoto.name}</p>}
            </div>
            <button
            ...
            >
              {isSubmittingDocs ? <><Loader2 className="w-4 h-4 animate-spin" /> Updating...</> : "Update documents"}
            </button>
          </div>
        </BottomPopup>
          ...
          <div className="bg-white rounded-lg shadow-sm divide-y divide-gray-200">
            <div className="p-2 px-3 flex items-center justify-between">
              <div className="w-full align-center flex content-center justify-between">
                ...
                <p className="text-base text-gray-900">{profile?.phone || "N/A"}</p>
              </div>
            </div>
            <div className="p-2 px-3 flex items-center justify-between">
              <div className="w-full align-center flex content-center justify-between">
            ...
            <div className="p-2 px-3 flex items-center justify-between">
              <div className="w-full align-center flex content-center justify-between">
                ...
                <p className="text-base text-gray-900">{profile?.documents?.aadhar?.number || "-"}</p>
              </div>
            </div>
            <div className="p-2 px-3 flex items-center justify-between">
              <div className="w-full align-center flex content-center justify-between">
                <p className="text-sm text-gray-900 mb-1">Rating</p>
                <p className="text-base text-gray-900">{ratingDisplay}</p>
              </div>
            </div>
            <div className="p-2 px-3 flex items-center justify-between">
              <div className="w-full align-center flex content-center justify-between">
                <p className="text-sm text-gray-900 mb-1">Wallet Balance</p>
                <p className="text-base text-gray-900">₹{Number(walletBalance || profile?.wallet?.balance || 0).toFixed(2)}</p>
              </div>
            </div>
            <div className="p-2 px-3 flex items-center justify-between">
              <div className="w-full align-center flex content-center justify-between">
                <p className="text-sm text-gray-900 mb-1">Status</p>
                <p className="text-base text-gray-900 capitalize">{profile?.status || "N/A"}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-lg shadow-sm divide-y divide-gray-200">
            <div className="p-2 px-3 flex items-center justify-between">
              <div className="w-full align-center flex content-center justify-between">
                ...
                <p className="text-base text-gray-900">{profile?.documents?.bankDetails?.accountHolderName || "-"}</p>
              </div>
            </div>
            <div className="p-2 px-3 flex items-center justify-between">
              <div className="w-full align-center flex content-center justify-between">
                <p className="text-sm text-gray-900 mb-1">Account Number</p>
                <p className="text-base text-gray-900">{profile?.documents?.bankDetails?.accountNumber ? `****${profile.documents.bankDetails.accountNumber.slice(-4)}` : "-"}</p>
              </div>
            </div>
            <div className="p-2 px-3 flex items-center justify-between">
              <div className="w-full align-center flex content-center justify-between">
                <p className="text-sm text-gray-900 mb-1">IFSC Code</p>
                <p className="text-base text-gray-900">{profile?.documents?.bankDetails?.ifscCode || "-"}</p>
              </div>
            </div>
            <div className="p-2 px-3 flex items-center justify-between">
              <div className="w-full align-center flex content-center justify-between">
                <p className="text-sm text-gray-900 mb-1">Bank Name</p>
                <p className="text-base text-gray-900">{profile?.documents?.bankDetails?.bankName || "-"}</p>
              </div>
            </div>
            <div className="p-2 px-3 flex items-center justify-between">
              <div className="w-full align-center flex content-center justify-between">
                <p className="text-sm text-gray-900 mb-1">Pan Card Number</p>
                <p className="text-base text-gray-900">{profile?.documents?.pan?.number || "-"}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <BottomPopup isOpen={showVehiclePopup} onClose={() => setShowVehiclePopup(false)} title="Vehicle Number">
        <div className="space-y-4">
          <input type="text" value={vehicleInput} onChange={(e) => setVehicleInput(e.target.value.toUpperCase())} placeholder="MH 12 AB 1234" className="w-full h-14 bg-gray-50 border border-transparent focus:border-green-500 rounded-lg px-4 text-lg font-bold outline-none" />
          ...
          }} className="w-full bg-black text-white py-4 rounded-lg font-bold uppercase tracking-widest">Update</button>
        </div>
      </BottomPopup>

      {/* Bank Details Edit Popup */}
      <BottomPopup isOpen={showBankDetailsPopup} onClose={() => setShowBankDetailsPopup(false)} title="Edit Bank Details">
        <div className="space-y-4 pb-4">
          {Object.entries({
            ...
            <div key={key}>
              ...
              <input type="text" value={bankDetails[key]} onChange={(e) => setBankDetails({...bankDetails, [key]: e.target.value})} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none" />
            </div>
          ))}
          <button onClick={async () => {
             ...
             finally { setIsUpdatingBankDetails(false) }
          }} disabled={isUpdatingBankDetails} className="w-full bg-[#00B761] text-white py-3 rounded-lg font-medium">Save Details</button>
        </div>
      </BottomPopup>
      ...
      {showDocumentModal && selectedDocument && (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-75 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-auto relative">
            <button onClick={() => setShowDocumentModal(false)} className="absolute top-4 right-4 z-10 bg-white rounded-full p-2 shadow-lg hover:bg-gray-100 transition-colors">
            ...
            <div className="p-4">
              <img src={selectedDocument.url} alt={selectedDocument.name} className="w-full h-auto rounded-lg" />
            </div>
          </div>
        </div>
      )}
