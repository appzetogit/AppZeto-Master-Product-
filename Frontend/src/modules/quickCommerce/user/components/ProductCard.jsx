...
const ProductCard = memo(({ product, quantity, onAdd, onIncrement, onDecrement, compact = false }) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group flex h-full flex-col overflow-hidden rounded-[24px] border border-[#f1f5f1] transition-all duration-300 hover:shadow-[0_15px_35px_rgba(0,0,0,0.08)]",
    ...
    >
      ...
      <div className="flex flex-1 flex-col px-3.5 pb-3.5">
        ...
        <div className="mt-auto flex items-end justify-between pt-3.5">
          ...
          <div className="flex">
            {quantity > 0 ? (
              <div className="flex items-center rounded-xl border border-[#0c831f] bg-white p-0.5 shadow-sm min-w-[80px] justify-between">
                <button 
                  onClick={(e) => { e.stopPropagation(); onDecrement(product); }} 
                  className="p-1 px-1.5 text-[#0c831f] transition-transform active:scale-90"
                >
                  <Minus className="h-3.5 w-3.5 stroke-[3.5px]" />
                </button>
                <span className="text-[13px] font-black text-[#0c831f]">{quantity}</span>
                <button 
              ...
              <motion.button
                ...
                className="rounded-xl border border-[#0c831f] bg-white px-6 py-2 text-[12px] font-black uppercase tracking-wide text-[#0c831f] shadow-sm transition-all hover:bg-[#0c831f]/5"
              >
                ADD
              </motion.button>
            )}
